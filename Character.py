...
class Character(Mover):
    ...
    def attack(self, event_position, attack_type):
        ...
        weapon = attack_type(player_to_click, self.rect, True, attack_type.ability_level)

        try: # For rejecting Fire
            if pygame.time.get_ticks() - self.timer > weapon.cooldown: # Limit attacks per second based on ability.
                if not self.energy - weapon.energy_consume < 0:
                    ...
                    if isinstance(weapon, Sword.Attack) or isinstance(weapon, Sword.FireStorm):
                        return weapon
                    # For Splitshot.
                    else:
    ...
    def handle_collision(self, collided_sprite):
        if not isinstance(collided_sprite, Sword.Attack) and not isinstance(collided_sprite, Sword.FireStorm):
                self.velocity = list(map(lambda x : -2 * x, self.velocity))

        if isinstance(collided_sprite, Sword.Attack) or isinstance(collided_sprite, Sword.FireStorm):
    ...
    def recover(self, toughen_up_level):
        if self.health < self.max_health:
            self.health += .1 + (.05 * toughen_up_level)
    ...
    def increment_maxes(self, toughen_up_level):
        current_max_health = self.max_health
        health_change = 50
        self.max_health = 700 + (health_change * toughen_up_level)
